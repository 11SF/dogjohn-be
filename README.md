...
- config ssh for private repo [GitLab SSH Keys Krungthai](https://gitdev.devops.krungthai.com/-/profile/keys). otherwise, you will not be able to install use `gonew` command to create a new project.
  - or use personal access token set in `~/.netrc` file

```.netrc
machine gitdev.devops.krungthai.com
login <account name>
...
1. make sure you set `go env -w GOPRIVATE=gitdev.devops.krungthai.com` (to allow go get private repository)
1. copy this repository and rename it to your project name or use `gonew github.com/11SF/dogjohn-be gitdev.devops.krungthai.com/your/service/module/xxxx.git`
...
1. add your git remote repository by running `git remote add origin gitdev.devops.krungthai.com/your/service/module/xxxx.git` (DO NOT FORGET TO CREATE YOUR REPOSITORY FIRST IN GitLab)
